...
const Bar = ({ data }) => {
  ...
  const fetchBarchart = async () => {
    ...
    //setBarchart(barchart.data)
    setBar(imageObjectURL)
  ...
  useEffect(() => {
    ...
    console.log("columnsinbar ", columns)
    console.log('bar', bar)

  }, [columns, bar])

  useEffect(() => {
    seturl('http://127.0.0.1:8000/bar/' + x + '/y/' + y + '/' + name)
    fetchBarchart();
  }, [x, y, url])
  ...
  const yaxisChange = event => {
    console.log("on change of y axis: ",event.target.value)
    if (event.target.value) {
